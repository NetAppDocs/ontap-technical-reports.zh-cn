...
在零信任模型的术语中，数据分类涉及有毒数据的识别。有毒数据是不打算暴露在组织外部的敏感数据。披露有毒数据可能会违反法规合规并损害组织的声誉。在合规性方面，有毒数据包括 https://www.netapp.com/pdf.html?item=/media/17180-tr4401.pdf["支付卡行业数据安全标准（ PCI-DSS ）"^]的持卡人数据、欧盟 https://www.netapp.com/responsibility/trust-center/data-privacy-principles-compliance/gdpr-compliance/["《一般数据保护条例》(GDPR)"^]的个人数据或 https://www.hhs.gov/hipaa/for-professionals/privacy/laws-regulations/index.html["健康保险携带和责任法案(HIPAA)"^]的医疗保健数据。您可以使用 NetApp https://www.netapp.com/data-services/classification/["NetApp Data Classification"^]（以前称为 Cloud Data Sense）这一人工智能驱动的工具包，自动扫描、分析和分类数据。
...
除了集群管理 RBAC 之外，https://www.netapp.com/pdf.html?item=/media/17055-tr4647.pdf["多因素身份验证(MFA)"^]还可以部署用于 ONTAP Web 管理访问和安全 Shell (SSH) 命令行访问。管理访问的 MFA 是美国公共部门组织或必须遵循 PCI-DSS 的组织的要求。MFA 使攻击者无法仅使用用户名和密码来入侵帐户。MFA 需要两个或多个独立因素才能进行身份验证。双因素身份验证的一个示例是用户拥有的东西（例如私钥）和用户知道的东西（例如密码）。通过安全断言标记语言 (SAML) 2.0 启用对 ONTAP System Manager 或 ActiveIQ Unified Manager 的管理 Web 访问。SSH 命令行访问使用带有公钥和密码的链式双因素身份验证。
...
每天都有新的要求来缓解存储系统风险和基础架构差距，当组织重新调整驱动器的用途、退回有缺陷的驱动器或通过出售或折价升级到更大的驱动器时。作为数据的管理员和操作员，存储工程师有望在数据的整个生命周期中安全地管理和维护数据。 https://www.netapp.com/pdf.html?item=/media/19767-ds-3898-netapp-storage-encryption-volume-encryption.pdf["NetApp存储加密(NSE)#44；NetApp卷加密(NVE)#44；以及NetApp聚合加密"^]帮助您始终对静态数据进行加密，无论其是否有毒，并且不会影响日常操作。 https://www.netapp.com/pdf.html?item=/media/7563-ds-3213-en.pdf["NSE"^]是一种 ONTAP 硬件link:https://docs.netapp.com/us-en/ontap/encryption-at-rest/index.html["空闲数据"^]解决方案，使用 FIPS 140-2 2 级验证的自加密驱动器。 https://www.netapp.com/pdf.html?item=/media/17070-ds-3899.pdf["NVE和NAE"^]是一种 ONTAP 软件link:https://docs.netapp.com/us-en/ontap/encryption-at-rest/index.html["空闲数据"^]解决方案，使用 https://csrc.nist.gov/projects/cryptographic-module-validation-program/certificate/4144["FIPS 140-2 1级验证的NetApp加密模块"^]。使用 NVE 和 NAE，可以使用硬盘驱动器或固态驱动器进行静态数据加密。此外，NSE 驱动器可用于提供本机分层加密解决方案，可提供加密冗余和额外的安全性。如果一层被破坏，则第二层仍会保护数据。这些功能使 ONTAP 在 https://www.netapp.com/pdf.html?item=/media/17002-sb-3952.pdf["量子就绪加密"^]方面处于有利地位。
...
NetApp 的 https://docs.netapp.com/us-en/data-infrastructure-insights/cs_intro.html["DII 存储工作负载安全"^]利用云和本地ONTAP存储系统上的 FPolicy 界面和用户行为分析，为您提供恶意用户行为的实时警报。存储工作负载安全通过先进的机器学习和异常检测保护组织数据不被恶意或受感染的用户滥用。存储工作负载安全可以识别勒索软件攻击或其他恶意行为，调用快照并隔离恶意用户。存储工作负载安全还具有取证功能，可以详细查看用户和实体活动。存储工作负载安全是NetAppData Infrastructure Insights的一部分。
