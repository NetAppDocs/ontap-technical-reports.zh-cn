...
link:https://docs.netapp.com/us-en/netapp-solutions-sap/backup/hana-sc-generic-scope.html["TR-4614 ：《使用 SnapCenter 实现 SAP HANA 备份和恢复》"^] SnapCenter 是一个统一、可扩展的平台，可为 SAP HANA 和其他数据库提供应用程序一致性数据保护。SnapCenter 提供集中控制和监督，同时让用户能够管理特定于应用程序的备份、还原和克隆作业。使用 SnapCenter，数据库和存储管理员可以学习单个工具来管理各种应用程序和数据库的备份、还原和克隆操作。
