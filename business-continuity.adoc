...
.MetroCluster IP
link:https://www.netapp.com/pdf.html?item=/media/13481-tr4689.pdf["TR-4689：NetApp MetroCluster IP"^] MetroCluster 是一种持续可用的存储解决方案，适用于在 FAS 和 AFF 系统上运行的 ONTAP。MetroCluster IP 是使用基于以太网的后端存储结构的最新演进。MetroCluster IP 提供高度冗余的配置，以满足最关键业务应用程序的需求。MetroCluster IP 包含在 ONTAP 中，为使用 ONTAP 存储的客户端和服务器提供 NAS 和 SAN 连接。
